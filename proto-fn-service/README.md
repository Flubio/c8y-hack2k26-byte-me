...
Functions are stored in SQLite (`FN_DB_PATH`, default `.data/functions.db`). The container filesystem is ephemeral on Cumulocity: the file survives process restarts but not a redeploy or reschedule, so redeploy demo functions after uploading a new version.
